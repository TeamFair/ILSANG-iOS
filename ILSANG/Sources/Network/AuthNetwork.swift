...
final class AuthNetwork {
    ...
    func login(idToken: String, channel: AuthChannel) async -> Result<String, NetworkError> {
        let body = [
        ...
        let result: Result<Response<AwsAuth>, Error> = await Network.requestData(url: url+"/oauth", method: .post, parameters: nil, body: bodyData, withToken: false)
        switch result {
        case .success(let res):
            return .success(res.data.authorization)
